...
public class GetInterests extends OpenlinkCommand {
	...
	public GetInterests(OpenlinkComponent OpenlinkComponent) {
	...
	public Element execute(SessionData data, Element newCommand, Element oldCommand)
	{
		try {
			...
			if (OpenlinkUser != null)
			{
				...
				Iterator it = OpenlinkUser.getInterests().values().iterator();

				while( it.hasNext() )
				{
					OpenlinkInterest OpenlinkInterest = (OpenlinkInterest)it.next();
					String interestID = OpenlinkInterest.getInterestId() + OpenlinkUser.getUserNo();
					...
					if (OpenlinkInterest.getUserInterests().containsKey(OpenlinkUser.getUserNo()))
					{
