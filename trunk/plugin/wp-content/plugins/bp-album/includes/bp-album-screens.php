...
function bp_album_screen_single() {
	...
	function bp_album_screen_edit_content() {
		...
		<form method="post" enctype="multipart/form-data" name="bp-album-edit-form" id="bp-album-edit-form" class="standard-form">
            <img id="picture-edit-thumb" width="128" src='<?php bp_album_picture_thumb_url() ?>' />
            <p>
...
function bp_album_screen_upload() {
	...
	function bp_album_screen_upload_content() {
		...
		<form method="post" enctype="multipart/form-data" name="bp-album-upload-form" id="bp-album-upload-form" class="standard-form">
            ...
            <input type="hidden" name="MAX_FILE_SIZE" value="<?php echo BP_AVATAR_ORIGINAL_MAX_FILESIZE; ?>" />
...
function bp_album_action_upload() {
	...
	if ( $bp->current_component == $bp->album->slug && $bp->album->upload_slug == $bp->current_action && isset( $_POST['submit'] )) {
		...
		$uploadErrors = array(
			...
			1 => __("Your image was bigger than the maximum allowed file size of: ", 'bp-album') . size_format(BP_AVATAR_ORIGINAL_MAX_FILESIZE),
			2 => __("Your image was bigger than the maximum allowed file size of: ", 'bp-album') . size_format(BP_AVATAR_ORIGINAL_MAX_FILESIZE),
		...
		if ( isset($_FILES['file']) ){
			...
			elseif ( $_FILES['file']['size'] > BP_AVATAR_ORIGINAL_MAX_FILESIZE ) {
				...
				$feedback_message[] = sprintf( __( 'The file you uploaded is too big. Please upload a file under %s', 'bp-album'), size_format(BP_AVATAR_ORIGINAL_MAX_FILESIZE) );
				$error_flag = true;
