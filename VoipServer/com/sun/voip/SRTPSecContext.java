...
public class SRTPSecContext {
    ...
    public SRTPSecContext(boolean in) {
        _in = in;
        _dirn = in ? "in " : "out ";
    }

    public SRTPSecContext() {
        this(false);
    }
    ...
    public static int copyBits(int input, int in_noLSB, byte output[], int out_pos)
    {
        ...
        for (int i=0; i<in_noLSB; i++) {
            ...
            res = (value & mask);
            if (res > 0) {
                SRTPSecContext.setBit(output, out_pos);
            }
    ...
    public static void setBit(byte output[], int bitno) throws java.lang.ArrayIndexOutOfBoundsException
    {
    ...
    public static short copyBits(byte input[], int in_pos, int no_bits)
    {
        ...
        for (int b=0; b<no_bits; b++) {
            myBit = SRTPSecContext.getBit(input, in_pos);
            if (myBit > 0) {
    ...
    public static int getBit(byte input[], int bitno) throws java.lang.ArrayIndexOutOfBoundsException
    {
        // bit 0 is on the left hand side, if bit 0 should be set to '1'
        // this would show as: 1000 0000 = 0x80
    ...
    public static void main(String argv[]) {
        ...
        try {
            Logger.println("test key derivation");
            s.testDeriv();
            s.testHmac();
    ...
    public void deriveKeys(long index) throws GeneralSecurityException {
    ...
    private void deriveKeys(long index, int kdr) throws GeneralSecurityException {
        ...
        _sessionAuth = getKeyBytes(myinpblk, MACKEYLEN);
            ...
            Logger.println(_dirn + "Derive keys for index = " + index + " kdr =" + kdr);
            Logger.println(_dirn + "Session Key = " + getHex(_sessionKey));
            Logger.println(_dirn + "CipherSalt Key = " + getHex(_cipherSalt));
            Logger.println(_dirn + "Auth Key = " + getHex(_sessionAuth));
    ...
    public Mac getAuthMac() throws GeneralSecurityException {

        //Logger.println(_dirn + "getting Hmac key = " + getHex(_sessionAuth));
    ...
    public void decipher(ByteBuffer in, ByteBuffer out, ByteBuffer pepper) throws GeneralSecurityException {
        ...
        aes.init(Cipher.ENCRYPT_MODE, keyp);
           ...
           // Logger.println(_dirn + "decipher: key    =" + getHex(_sessionKey));
        ...
        ByteBuffer csalt = ByteBuffer.wrap(_cipherSalt);
            ...
            //Logger.println(_dirn + "decipher: salt   =" + getHex(csalt.array()));
            //Logger.println(_dirn + "decipher: pepper =" + getHex(pepper.array()));



        bbxor(pepper, csalt);

            //Logger.println(_dirn + "decipher: IV     =" + getHex(pepper.array()));
            //Logger.println(_dirn + "decipher: in     =" + getHex(in.array()));

        getCypherStreamBytes(aes, pepper, out);

            //Logger.println(_dirn + "decipher: stream =" + getHex(out.array()));


        bbxor(out, in);

            //Logger.println(_dirn + "decipher: xor    =" + getHex(out.array()));
    ...
    public void parseCryptoProps(Properties cryptoProps) throws GeneralSecurityException {
        /*
        ...
        if ((key_params != null) && (key_params.startsWith("inline:"))) {
            ...
            Logger.println(_dirn + "key n salt =" + ks);
            ...
            if (mkil != null) {
                ...
                if (mbits.length == 2) {
                    ...
                    Logger.println(_dirn + "MKI:length defined");
                } else {
            ...
            if (life != null) {
                ...
                Logger.println("Key life is " + _keyLife);
...
// now do something with the session params and the tag.
        if (session_params != null) {
            ...
            for (int i = 0; i < sbits.length; i++) {
                Logger.println(_dirn + "SDP session param " + sbits[i]);
                ...
                if (sbits[i].startsWith("KDR=")) {
                    ...
                    Logger.println(_dirn + "setting kdr to " + _kdr);
                    if (_kdr != 0) {
                        Logger.println(_dirn + "Note: Non-zero kdr");
                    }
                }
                if ("UNENCRYPTED_SRTP".equals(sbits[i])) {
                    // - :      SRTP messages are not encrypted.
                    _doCrypt = false;
                    Logger.println(_dirn + "Switching off encryption - is this what you wanted ?");
                }
                if ("UNENCRYPTED_SRTCP".equals(sbits[i])) {
                    //SRTCP messages are not encrypted.
                    Logger.println(_dirn + "No support for SRTCP (yet) so SDP setting ignored");
                }
        ...
        if (tag != null) {
            ...
            Logger.println(_dirn + "SDP tag = " + _tag);
        }
    }
    /*
