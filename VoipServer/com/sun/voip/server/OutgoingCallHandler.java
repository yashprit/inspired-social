...
public class OutgoingCallHandler extends CallHandler
	...
	onlyOneGateway = false;
        ...
        } else if (cp.getPhoneNumber() != null && cp.getPhoneNumber().indexOf("jingle:") == 0) {

			 placeCall(); // jingle call

		} else if (cp.getProtocol() != null &&  (cp.getProtocol().equalsIgnoreCase("RTMP") || cp.getProtocol().equalsIgnoreCase("RTMFP"))) {
	...
	} else if (protocol.equalsIgnoreCase("JINGLE")) {
	    csa = new JingleOutgoingCallAgent(this);
	} else {
    ...
    public boolean waitForCallToBeAnswered() {
		...
		if (protocol.equalsIgnoreCase("RTMP") || protocol.equalsIgnoreCase("RTMFP")) {
			return true;
		}
    ...
    public boolean waitForCallToBeEstablished() {
		...
		if (cp.getProtocol().equalsIgnoreCase("RTMP") || cp.getProtocol().equalsIgnoreCase("RTMFP")) {
			return true;
		}
