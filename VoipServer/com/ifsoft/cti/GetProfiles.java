...
{
	public GetProfiles(OpenlinkComponent openlinkComponent) {
		super(openlinkComponent);

		openlinkComponent.getUserProfiles();
	}
	...
	public Element execute(SessionData data, Element newCommand,Element oldCommand)
	{
		...
		try {
			...
			List<OpenlinkUser> openlinkUsers = this.getOpenlinkComponent().getOpenlinkProfiles(userJID);
			Iterator it = openlinkUsers.iterator();

			while( it.hasNext() )
			{
				OpenlinkUser openlinkUser = (OpenlinkUser)it.next();

				String profileID = openlinkUser.getUserNo();

				Element genericProfile = profiles.addElement("profile");
				genericProfile.addAttribute("id", profileID);
				genericProfile.addAttribute("label", openlinkUser.getUserName());
				genericProfile.addAttribute("default", openlinkUser.getDefault());
				genericProfile.addAttribute("online", "0.0.0.0".equals(openlinkUser.getDeviceNo()) ? "false" : "true");
				...
				genericActions.addElement("action").addAttribute("id","SendDigopenlink").addAttribute("label",		"Send dial digits on selected line");
