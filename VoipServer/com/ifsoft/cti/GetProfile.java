...
public class GetProfile extends OpenlinkCommand {
	...
	public GetProfile(OpenlinkComponent openlinkComponent) {
	...
	public Element execute(SessionData data, Element newCommand, Element oldCommand)
	{
		try {
			...
			if (openlinkUser != null)
			{
				...
				if(!AdminManager.getInstance().isUserAdmin(userAgent, false) && !userAgent.equals(openlinkUser.getUserId()) && !userAgent.equals(openlinkUser.getUserNo()))
				{
				...
				profile.addAttribute("online", "0.0.0.0".equals(openlinkUser.getDeviceNo()) ? "false" : "true");
