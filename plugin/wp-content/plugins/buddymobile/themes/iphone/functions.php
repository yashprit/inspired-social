...
wp_enqueue_script( 'mobile-theme-js', get_template_directory_uri() . '/theme.js', array( 'jquery' ) );

if ( !empty( $buddymobile_options['add2homescreen'] ) ) {
		wp_enqueue_script( 'addhome-theme-js', get_template_directory_uri() . '/add2home.js', array( 'jquery' ) );
}

if ( $buddymobile_options['theme-style'] == 'dark' ) {
		wp_enqueue_style( 'black', get_template_directory_uri()  . '/black.css' );
}
...
function buddymobile_custom_styles() {
	...
	if ( !empty( $buddymobile_options['toolbar-color'] ) ) {
		...
		echo "<style type='text/css'>";
			...
			echo "#content a, #footer a { color: $color !important; }";
