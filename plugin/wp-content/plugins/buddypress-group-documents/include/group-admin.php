...
function bp_group_documents_group_admin_nav() {
	...
	add_action( 'groups_admin_tabs', create_function( '$current, $group_slug', 'if ( "' . esc_attr( $bp->group_documents->slug ) . '" == $current ) $selected = " class=\"current\""; echo "<li{$selected}><a href=\"' . $bp->root_domain . '/' . $bp->groups->slug . '/{$group_slug}/admin/' . esc_attr( $bp->group_documents->slug ) . '\">' . __('Documents','bp-group-documents') . '</a></li>";' ), 10, 2 );
		...
		<div>
			...
			<?php foreach( $group_categories as $category ) {
				if( isset( $_GET['edit'] ) && ( $_GET['edit'] == $category->term_id ) ) { ?>
					...
					<input type="submit" value="<?php _e('Update');?>" /></li>
				<?php } else {
			...
			<?php } ?>
				...
				<input type="submit" value="<?php  _e('Add');?>" /></li>
			</ul>
		</div>
...
function bp_group_documents_group_admin_save() {
	...
	if ( $success !== false ) {
		bp_core_add_message( __( 'Settings saved successfully', 'bp-group-documents') );
